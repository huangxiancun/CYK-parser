...
方法描述：
	学习文法：
		...
		Lex是一个正则表达式解析器框架，它读入语法树文件，根据正则匹配将文本分割成带值的标记（token），比如左括号'('(代码中为LP)，右括号')'(代码中为RP)，其他符号都解析为TOKEN标记，它们的值都是原文本。
		Yacc是一个移进-规约CFG解析器框架，它读入Lex产生的带值的标记序列，根据语法树的文法解析标记序列，它每采用一个产生式进行规约，就执行相应的语义过程————即输出学习到的产生式。产生式由标记的值（原文本）和‘->’组成。
		Yacc代码learn/parse.y使用这个CFG识别语法树结构：({F,S,X},{TOKEN,(,)},{F->S|FS|(S),S->(S)|(TOKEN C),C->TOKEN|X,X->S|XS},F)
